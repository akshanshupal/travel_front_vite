...
export default function ItineraryAreaAddPage() {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            useStoreSnackbar.getState().showSnackbar({ title: "Success", description: "Area created", color: "success" });
            navigate("/itinerary/area");
        } catch (e: any) {
            useStoreSnackbar.getState().showSnackbar({ title: "Error", description: e?.message || "Failed to create area", color: "danger" });
        } finally {
            setSaving(false);
        }
    };
