...
export default function ItinerarySiteEditPage() {
        ...
        const run = async () => {
            ...
            try {
                ...
                setForm({
                    ...
                    area: String(resolved?.area?.id ?? resolved?.area?._id ?? resolved?.area ?? ""),
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            useStoreSnackbar.getState().showSnackbar({ title: "Success", description: "Site updated", color: "success" });
            navigate("/itinerary/site");
        } catch (e: any) {
            useStoreSnackbar.getState().showSnackbar({ title: "Error", description: e?.message || "Failed to update site", color: "danger" });
        } finally {
            setSaving(false);
        }
    };
                    ...
                    <div className="space-y-4 bg-primary px-4 py-5 md:px-6">
                        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                            ...
                            <Select
                                ...
                                selectedKey={form.area}
                                onSelectionChange={(key) => setForm((p) => ({ ...p, area: String(key) }))}
                                items={areas}
                            >
