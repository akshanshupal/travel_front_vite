...
export default function ItineraryHotelCategoryEditPage() {
    ...
    useEffect(() => {
        const run = async () => {
            if (!id) {
                setLoadError("Invalid category id");
                setLoading(false);
                return;
            }
            setLoading(true);
            setLoadError(null);
            try {
                const res = await fetchWithToken(`/api/hotelcategory/${id}`, {});
                ...
                setLoadError(e?.error?.message || e?.message || "Failed to load category");
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            useStoreSnackbar.getState().showSnackbar({ title: "Success", description: "Category updated", color: "success" });
            navigate("/itinerary/hotel/category");
        } catch (e: any) {
            useStoreSnackbar.getState().showSnackbar({ title: "Error", description: e?.message || "Failed to update category", color: "danger" });
        } finally {
            setSaving(false);
        }
    };
    ...
    const breadcrumbs = (
        <div className="mb-4 rounded-sm border border-secondary bg-primary p-2">
            <div className="flex flex-wrap items-center gap-1 text-sm text-tertiary">
                ...
                <span>/</span>
                <button type="button" onClick={() => navigate("/itinerary/hotel/category")} className="rounded-sm px-1 py-0.5 text-primary hover:bg-primary_hover">
                    Hotel Category
                </button>
                <span>/</span>
                ...
                <TableCard.Header
                    ...
                    contentTrailing={
                        <div className="flex w-full flex-col gap-2 md:w-auto md:flex-row md:items-center md:justify-end">
                            <Button color="secondary" iconLeading={ArrowLeft} onClick={() => navigate("/itinerary/hotel/category")}>
                                Back
                            </Button>
