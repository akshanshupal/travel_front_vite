...
export default function TemplateEditPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!id) return;
            const response = await updateMailtemplateById(id, finalData);
            if (response && !response.error) {
                showSnackbar({ title: "Success", description: "Template updated successfully", color: "success" });
            ...
            showSnackbar({ title: "Error", description: error.message || "Failed to update template", color: "danger" });
        } finally {
            setIsLoading(false);
        }
    };
                    ...
                    <TableCard.Header
                        ...
                        contentTrailing={
                            <div className="flex w-full flex-col gap-2 md:w-auto md:flex-row md:items-center md:justify-end">
                                ...
                                <Button color="primary" isLoading={isLoading} isDisabled={!canEdit} onClick={() => formRef.current?.requestSubmit()}>
                                    Update
                                </Button>
                            </div>
                        }
                    ...
                    <div className="flex flex-col gap-6">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div className="flex flex-col gap-2">
                                ...
                                {formData.logo && (
                                    <div className="relative group size-32 mb-2">
                                        <img src={getFilePreview(formData.logo)!} alt="Logo" className="size-full object-cover rounded-lg border" />
                                        <button 
                                            type="button" 
                                            onClick={() => handleChange("logo", null)}
                                            className="absolute -top-2 -right-2 rounded-full bg-primary p-1 text-primary ring-1 ring-secondary hover:text-error-primary"
                                        >
                            ...
                            <div className="flex flex-col gap-2">
                                ...
                                {formData.emailIcon && (
                                    <div className="relative group size-32 mb-2">
                                        <img src={getFilePreview(formData.emailIcon)!} alt="Email Icon" className="size-full object-cover rounded-lg border" />
                                        <button 
                                            type="button" 
                                            onClick={() => handleChange("emailIcon", null)}
                                            className="absolute -top-2 -right-2 rounded-full bg-primary p-1 text-primary ring-1 ring-secondary hover:text-error-primary"
                                        >
                                            <Trash01 className="size-4" />
                                        </button>
                            ...
                            <div className="flex flex-col gap-2">
                                ...
                                {formData.userIcon && (
                                    <div className="relative group size-32 mb-2">
                                        <img src={getFilePreview(formData.userIcon)!} alt="User Icon" className="size-full object-cover rounded-lg border" />
                                        <button 
                                            type="button" 
                                            onClick={() => handleChange("userIcon", null)}
                                            className="absolute -top-2 -right-2 rounded-full bg-primary p-1 text-primary ring-1 ring-secondary hover:text-error-primary"
                                        >
                                            <Trash01 className="size-4" />
                                        </button>
                                ...
                                {formData.paymentType.map((item, index) => (
                                    <div key={index} className="flex flex-col gap-4 p-4 border rounded-lg border-secondary relative">
                                        ...
                                        <div className="flex flex-col gap-2">
                                            ...
                                            {item.paymentImage && (
                                                <div className="relative group size-24 mb-2">
                                                    <img src={getFilePreview(item.paymentImage)!} alt="Payment" className="size-full object-cover rounded-lg border" />
                                                    <button 
                                                        type="button" 
                                                        onClick={() => handlePaymentChange(index, "paymentImage", null)}
                                                        className="absolute -top-2 -right-2 rounded-full bg-primary p-1 text-primary ring-1 ring-secondary hover:text-error-primary"
                                                    >
                                                        <Trash01 className="size-4" />
                                                    </button>
