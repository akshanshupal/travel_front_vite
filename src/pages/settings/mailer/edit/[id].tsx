...
export default function SettingsMailerEditPage() {
    ...
    useEffect(() => {
        const run = async () => {
            ...
            setLoading(true);
            try {
                const [mailerResponse, companyResponse] = await Promise.all([
                    getMailerById(id),
                    getCompanyConfig({ populate: "company" }).catch(() => null),
                ]);

                const resolvedCompany = (companyResponse as any)?.data ?? companyResponse;
                setCompanyConfig(Array.isArray(resolvedCompany) ? resolvedCompany[0] : resolvedCompany);

                const data = (mailerResponse as any)?.data ?? mailerResponse;
                const mailer = Array.isArray(data) ? data[0] : data;
                setForm({
                    title: mailer?.title || "",
                    host: mailer?.host || "",
                    email: mailer?.email || "",
                    password: mailer?.password || "",
                    emailFunction: mailer?.emailFunction || "",
                    subject: mailer?.subject || "",
                    html: mailer?.html || "",
                    status: String(Boolean(mailer?.status)),
                });
            } catch (error: any) {
...
<p style="color: #555; font-size: 14px; margin-top: 15px;"><strong>${companyName}</strong><br/>Address: ${companyAddress}<br/>Email: <a href="mailto:${companyEmail}" style="color: #1a73e8; text-decoration: none;">${companyEmail}</a></p>
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const response: any = await updateMailerById(id, payload);
            if (response?.error) throw new Error(response.error);
            showSnackbar({ title: "Success", description: "Mailer updated successfully", color: "success" });
            navigate("/settings/mailer");
        } catch (error: any) {
            showSnackbar({ title: "Error", description: error?.message || "Failed to update mailer", color: "danger" });
        } finally {
            setSaving(false);
        }
    };
    ...
    return (
        <DefaultLayout>
            <div className="space-y-4" style={{ width: availableWidth }}>
                ...
                <TableCard.Root>
                    <TableCard.Header
                        title={loading ? "Edit Mailer" : `Edit: ${form.title || "Mailer"}`}
                        contentTrailing={
