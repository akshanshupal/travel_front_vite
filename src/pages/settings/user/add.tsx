...
export default function SettingsUserAddPage() {
    ...
    const validate = () => {
        ...
        if (form.password && form.password.length < 6) next.password = "Password must be at least 6 characters long";
        setErrors(next);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (response?.error) throw new Error(response.error);
            showSnackbar({ title: "Success", description: "User added successfully", color: "success" });
            navigate("/settings/user");
        } catch (e: any) {
            showSnackbar({ title: "Error", description: e?.message || "Failed to add user", color: "danger" });
        } finally {
            setSaving(false);
        }
    };
                        ...
                        <div className="space-y-2">
                            ...
                            {profileImgPreview ? (
                                <div className="relative overflow-hidden rounded-xl ring-1 ring-secondary">
                                    <img src={profileImgPreview} alt="Profile preview" className="max-h-80 max-w-100 object-cover" />
