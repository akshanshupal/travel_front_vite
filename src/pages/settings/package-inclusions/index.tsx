...
export default function SettingsPackageInclusionsListPage() {
    ...
    const { can } = useAccess();
    const canView = can("packageinclusion", "view");
    const canAdd = can("packageinclusion", "add");
    const canEdit = can("packageinclusion", "edit");
    const canDelete = can("packageinclusion", "delete");
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            if (response && !(response as any).error) {
                ...
                showSnackbar({ title: "Error", description: (response as any)?.error || "Failed to fetch package inclusions", color: "danger" });
            }
        } catch (error) {
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            if (response && !(response as any).error) {
                showSnackbar({ title: "Success", description: "Package inclusion deleted successfully", color: "success" });
                fetchData();
                setDeleteModalOpen(false);
            } else {
                showSnackbar({ title: "Error", description: (response as any)?.error || "Failed to delete package inclusion", color: "danger" });
            }
        } catch {
            ...
            <div className="space-y-4" style={{ width: availableWidth }}>
                <TableCard.Root className="w-full">
                    <TableCard.Header
                        ...
                        description="Manage package inclusions under settings."
                        contentTrailing={
                            <div className="flex w-full flex-col gap-2 md:w-auto md:flex-row md:items-center">
                                ...
                                />
                                <Button color="primary" size="sm" iconLeading={Plus} isDisabled={!canAdd} onClick={() => navigate("/additional-data/settings/package-inclusions/add")}>
                                    ...
                                    {(column) => (
                                        <Table.Cell className={column.id === "actions" ? "whitespace-nowrap px-2 py-3" : "whitespace-normal break-words px-4 py-3"}>
                                            {column.id === "index" ? (
                                                ...
                                                <button type="button" className="text-left text-sm font-semibold text-primary hover:underline" disabled={!canView} onClick={() => navigate(`/additional-data/settings/package-inclusions/view/${getItemId(item)}`)}>
                                                    {item.title || "—"}
                                                </button>
                                            ) : column.id === "alias" ? (
                                                ...
                                                <div className="flex w-full items-center justify-end gap-1.5">
                                                    <ButtonUtility tooltip="View" tooltipPlacement="bottom" icon={Eye} onClick={() => navigate(`/additional-data/settings/package-inclusions/view/${getItemId(item)}`)} color="secondary" size="sm" isDisabled={!canView} />
                                                    <ButtonUtility tooltip="Edit" tooltipPlacement="bottom" icon={Edit01} onClick={() => navigate(`/additional-data/settings/package-inclusions/edit/${getItemId(item)}`)} color="secondary" size="sm" isDisabled={!canEdit} />
                                                    <ButtonUtility
                                                        tooltip="Delete"
                            ...
                            <div className="relative w-full rounded-xl bg-primary p-5 ring-1 ring-secondary">
                                ...
                                <div className="space-y-4">
                                    <div className="space-y-1">
                                        <h2 className="text-lg font-semibold text-primary">Delete Package Inclusion</h2>
