...
export default function SettingsPackageInclusionsEditPage() {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            showSnackbar({ title: "Success", description: "Package inclusion updated", color: "success" });
            navigate("/additional-data/settings/package-inclusions");
        } catch (e: any) {
            showSnackbar({ title: "Error", description: e?.message || "Failed to update package inclusion", color: "danger" });
        } finally {
            setSaving(false);
        }
    };
                ...
                <TableCard.Root>
                    ...
                    <div className="space-y-6 bg-primary px-4 py-5 md:px-6">
                        <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
                            <Input label="Title *" placeholder="Enter inclusion title" value={title} onChange={setTitle} isInvalid={Boolean(errors.title)} hint={errors.title} />
                            <Input label="Alias *" placeholder="Enter inclusion alias" value={alias} onChange={setAlias} isInvalid={Boolean(errors.alias)} hint={errors.alias} />
                            <Select
                                label="Status *"
                                selectedKey={status}
