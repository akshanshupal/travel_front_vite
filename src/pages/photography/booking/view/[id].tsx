...
const formatEventTiming = (event: any) => {
    const startTime = String(event?.startTime || event?.timing || "").trim();
...
const formatEventDuration = (event: any) => {
    ...
    const unit = String(event?.durationType || "").trim();
    if (duration === undefined || duration === null || duration === "") return "-";
    return unit ? `${duration} ${unit}` : String(duration);
};

const formatEventDeliverable = (event: any) => {
    const deliverable = event?.deliverable ?? event?.deliverables;
    if (Array.isArray(deliverable)) {
        const values = deliverable
...
export default function PhotographyBookingViewPage() {
            ...
            <TableCard.Root>
                <TableCard.Header
                    ...
                    contentTrailing={
                        <div className="flex gap-2">
                            <Button color="secondary" onClick={() => navigate("/photography/booking")}>
                                Back
                            </Button>
                            {activeTab === "payments" ? (
                ...
                <div className="space-y-4 bg-primary px-4 py-5 md:px-6">
                    <Tabs selectedKey={activeTab} onSelectionChange={(key) => setActiveTab(String(key))}>
                        ...
                        <Tabs.Panel id="bookingDetails" className="mt-4 space-y-4">
                            {loading ? (
                                <div className="text-sm text-tertiary">Loading booking...</div>
                            ) : !booking ? (
                                ...
                                <>
                                    <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
                                        ...
                                        <div className="rounded border border-secondary p-3">
                                            <div className="text-xs text-tertiary">Created At</div>
                                            <div className="text-sm font-semibold text-primary">{formatDateTime(booking?.createdAt)}</div>
                                        </div>
                                    </div>
                                    <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
                                        <div className="rounded border border-secondary p-3">
                                        ...
                                        <div className="rounded border border-secondary p-3">
                                            <div className="text-xs text-tertiary">Payment Status</div>
                                    ...
                                    <div className="grid grid-cols-1 gap-3 md:grid-cols-4">
                                        <div className="rounded border border-secondary p-3">
                                            ...
                                            <div className="text-sm font-semibold text-primary">₹{Number(booking?.amount || 0).toFixed(2)}</div>
                                        </div>
                                        <div className="rounded border border-secondary p-3">
                                            <div className="text-xs text-tertiary">GST</div>
                                            <div className="text-sm font-semibold text-primary">₹{Number(booking?.gst || 0).toFixed(2)}</div>
                                        </div>
                                        <div className="rounded border border-secondary p-3">
                        ...
                        <Tabs.Panel id="payments" className="mt-4 space-y-4">
                            {loading ? (
