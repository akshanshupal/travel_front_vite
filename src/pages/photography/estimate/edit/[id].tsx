...
export default function PhotographyEstimateEditPage() {
    ...
    useEffect(() => {
        const run = async () => {
            setLoading(true);
            try {
                const [clientsRes, estimateRes, agentsRes]: any = await Promise.all([
                    getPhotographyClients({ limit: "all" }),
                    getPhotographyEstimateById(id),
                    getUser({ limit: "all", type: "AGENT", select: "name,email,type" }),
                ]);
                ...
                const agentsResolved = agentsRes?.data ?? agentsRes;
                const agentList = Array.isArray(agentsResolved?.data) ? agentsResolved.data : Array.isArray(agentsResolved) ? agentsResolved : [];
                setAgents(
                    agentList
                        .map((item: any) => ({
                ...
                setSelectedAgent(String(estimate?.agent?.id || estimate?.agent || ""));
