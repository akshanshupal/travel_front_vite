...
import { createPhotographyEstimate, getPhotographyEstimates } from "@/utils/services/photographyEstimateService";
...
export default function PhotographyEstimatePage() {
    ...
    const fetchEstimates = async () => {
        try {
            const response: any = await getPhotographyEstimates({ limit: "10", totalCount: "true" });
            const resolved = response?.data ?? response;
            const list = Array.isArray(resolved?.data) ? resolved.data : Array.isArray(resolved) ? resolved : [];
    ...
    return (
        <DefaultLayout>
            <div className="space-y-4">
                <TableCard.Root>
                    <TableCard.Header
                        title="Photography - Estimate Creation"
                        contentTrailing={
                            <Button color="secondary" onClick={() => navigate("/photography/estimate/add")}>
                                Open Add Page
                            </Button>
                        }
                    />
                            ...
                            {items.map((item, index) => (
                                <div key={index} className="grid grid-cols-1 gap-3 rounded-lg border border-secondary p-3 md:grid-cols-2">
                                    ...
                                    <Input
                                        label="Timing"
                                        value={item.timing}
                                        onChange={(value) => updateItem(index, { timing: value })}
                                    />
                                    <TextArea
                ...
                <TableCard.Root>
                    <TableCard.Header title="Layout Preview" />
                    <div className="space-y-4 bg-white px-4 py-5 text-sm text-gray-800 md:px-6">
                        <div className="flex items-start justify-between border-b border-gray-300 pb-3">
                            <div className="flex h-14 w-24 items-center justify-center border border-gray-400 text-xs">Company Logo</div>
                            <div className="max-w-xl text-center">
                                <div className="text-lg font-bold"># CAPTURING MOMENTS-ESTIMATE</div>
                                <div>For any Service Related Issues Mail:- sales@capturingmoments.co.in</div>
                                <div>
                                    Capturing Moments Office No-A-1106, Tower - T3 , Block A, NX One, Noida Extention Noida, Uttar Pradesh
                                    201306, India
                                </div>
                                <div>8860030886 | capturing-moment-website.vercel.app</div>
                            </div>
                            <div />
                        </div>
                        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                            ...
                            <div className="space-y-1">
                                <div>Estimate Number: {estimateNumber || "-"}</div>
                                <div>Estimate Date: {formatDate(estimateDate) || "-"}</div>
                                <div>Valid Until: {formatDate(validUntil) || "-"}</div>
                                <div>Grand Total (INR): {grandTotal.toFixed(2)}</div>
                            </div>
                        </div>
                        <table className="w-full border-collapse">
                            <thead>
                                <tr className="bg-gray-200">
                                    <th className="border border-gray-300 px-2 py-2 text-left">Items</th>
                                ...
                                {items.map((item, index) => (
                                    <tr key={index}>
                                        <td className="border border-gray-300 px-2 py-2">
                                            ...
                                            {item.deliverables.map((entry, row) => (
                                                <div key={row}>{entry}</div>
                                            ))}
                                            <div>Package Cost 1</div>
                                        </td>
                                        <td className="border border-gray-300 px-2 py-2">{item.packageCost}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                        ...
                        <div>This is an electronically generated report, hence does not require a signature.</div>
                    </div>
                </TableCard.Root>

                <TableCard.Root>
                    <TableCard.Header title="Recent Estimates" badge={savedEstimates.length} />
