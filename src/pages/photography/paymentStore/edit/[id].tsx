...
import { getPhotographyPaymentStoreById, updatePhotographyPaymentStoreById } from "@/utils/services/photographyPaymentStoreService";
import { ArrowLeft, Home04 } from "@untitledui/icons";
import { useEffect, useMemo, useState } from "react";
import { useNavigate, useParams } from "react-router";

export default function PaymentStoreEditPage() {
    ...
    useEffect(() => {
        const run = async () => {
            ...
            setLoading(true);
            try {
                const response = await getPhotographyPaymentStoreById(id);
                if (response?.error) throw new Error(response.error);
                const resolved = response?.data ?? response;
                setForm({
                    title: String(resolved?.title ?? ""),
                    description: String(resolved?.description ?? ""),
                    isDefault: String((resolved?.isDefault ?? false) ? "true" : "false"),
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const response = await updatePhotographyPaymentStoreById(id, payload);
            if (response?.error) throw new Error(response.error);
            showSnackbar({
                title: "Success",
                description: "Payment store updated",
                color: "success",
            });
            navigate("/photography/paymentStore");
        } catch (error: any) {
            showSnackbar({
                title: "Error",
                description: error?.message || "Failed to update payment store",
                color: "danger",
            });
        } finally {
            setSaving(false);
    ...
    const breadcrumbs = (
        <div className="mb-4 rounded-sm border border-secondary bg-primary p-2">
            <div className="flex flex-wrap items-center gap-1 text-sm text-tertiary">
                ...
                <span>/</span>
                <button type="button" onClick={() => navigate("/photography/paymentStore")} className="rounded-sm px-1 py-0.5 text-primary hover:bg-primary_hover">
                    Payment Store
                </button>
                <span>/</span>
    ...
    return (
        <DefaultLayout>
            ...
            <TableCard.Root>
                <TableCard.Header
                    title="Edit Payment Store"
                    contentTrailing={
                        <div className="flex w-full flex-col gap-2 md:w-auto md:flex-row md:items-center md:justify-end">
                            <Button color="secondary" iconLeading={ArrowLeft} onClick={() => navigate("/photography/paymentStore")}>
                                Back
                            </Button>
                        ...
                        <>
                            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                                ...
                                <Select
                                    ...
                                    onChange={(value) => setForm((prev) => ({ ...prev, isDefault: String(value) }))}
                                    items={[
                                        { id: "true", label: "Yes" },
                                        { id: "false", label: "No" },
                                    ]}
                                ...
                                <Select
                                    label="Status *"
                                    value={form.status}
                                    onChange={(value) => setForm((prev) => ({ ...prev, status: String(value) }))}
                                    items={[
