...
import { getPaymentStore, getPaymentStoreDelete } from "@/utils/services/paymentStoreService";
...
export default function PaymentStorePage() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const response = await getPaymentStore(params);
            if (response.error) {
    ...
    const handleDelete = async () => {
        ...
        try {
            const response = await getPaymentStoreDelete(deleteTarget.id, {});
            if (response.error) {
                throw new Error(response.error);
            }
            showSnackbar({
                        ...
                        contentTrailing={
                            <div className="flex w-full flex-col gap-2 md:w-auto md:flex-row md:items-center">
                                ...
                                <Button
                                    ...
                                    color="primary"
                                    iconLeading={Plus}
                                    onClick={() => navigate("/bookings/paymentStore/add")}
                                >
                                    Add Payment Store
                                </Button>
                            </div>
                                                    ...
                                                    <div className="flex flex-col gap-4">
                                                        ...
                                                        <div className="flex flex-col gap-1.5">
                                                            ...
                                                            <Select
                                                                ...
                                                                onChange={undefined}
                                                                onSelectionChange={(key) => setTempFilters((prev) => ({ ...prev, isDefault: key === "__all__" ? "" : String(key) }))}
                                                                items={[
                                        ...
                                        {column.id === "index" ? (
                                            ...
                                            <div className="flex w-full items-center justify-end gap-1.5">
                                                <ButtonUtility
                                                    ...
                                                    icon={Eye}
                                                    onClick={() => navigate(`/bookings/paymentStore/view/${getItemId(item)}`)}
                                                    color="secondary"
                                                    size="sm"
                                                />
                                                <ButtonUtility
                                                    ...
                                                    tooltipPlacement="bottom"
                                                    icon={Edit01}
                                                    onClick={() => navigate(`/bookings/paymentStore/edit/${getItemId(item)}`)}
                                                    color="secondary"
                                                    size="sm"
                                                />
                                                <ButtonUtility
