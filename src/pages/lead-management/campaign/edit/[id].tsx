...
export default function CampaignEditPage() {
        ...
        const run = async () => {
            ...
            try {
                ...
                if (data) {
                    setForm({
                        title: data.title || "",
                        pipeline: data.pipeline?.id || data.pipeline || "",
                        managingCampaign: asArray(data.managingCampaign).map((v: any) => getId(v)).filter(Boolean),
                        salesExecutive: asArray(data.salesExecutive).map((v: any) => getId(v)).filter(Boolean),
    ...
    const handleSave = async () => {
        ...
        setSaving(true);
        try {
            const res = await updateCampaignById(id, form);
