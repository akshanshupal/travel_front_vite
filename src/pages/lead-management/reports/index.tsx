...
export default function LeadReportsPage() {
    ...
    return <DefaultLayout><div className="space-y-5">
        ...
        <TableCard.Root><TableCard.Header title="Agent Performance" />
            <FloatingHeaderTable><table className="w-full min-w-[50rem] text-left text-sm"><thead className="border-b border-secondary bg-secondary"><tr>{["Agent", "Total", "Open", "Converted", "Lost", "Conversion", "Pending Follow-ups"].map((label, labelIndex) => <th key={label} className={labelIndex === 0 ? "sticky left-0 z-20 bg-secondary px-4 py-3 font-semibold text-primary shadow-[4px_0_8px_-6px_rgba(0,0,0,0.35)]" : "bg-secondary px-4 py-3 font-semibold text-primary"}>{label}</th>)}</tr></thead><tbody>{agents.length ? agents.map((row, index) => <tr key={`${row.agent?.email || row.name}-${index}`} className="border-b border-secondary"><td className="sticky left-0 z-10 bg-primary px-4 py-3 shadow-[4px_0_8px_-6px_rgba(0,0,0,0.35)]"><div className="font-medium text-primary">{row.agent?.name || row.name || "Unassigned"}</div><div className="text-xs text-tertiary">{row.agent?.email || ""}</div></td><td className="px-4 py-3 text-secondary">{row.totalLeads || 0}</td><td className="px-4 py-3 text-secondary">{row.openLeads || 0}</td><td className="px-4 py-3 text-secondary">{row.convertedLeads || 0}</td><td className="px-4 py-3 text-secondary">{row.lostLeads || 0}</td><td className="px-4 py-3 text-secondary">{Number(row.conversionRate || 0).toFixed(1)}%</td><td className="px-4 py-3 text-secondary">{row.pendingFollowUps || 0}</td></tr>) : <tr><td colSpan={7} className="px-4 py-10 text-center text-tertiary">No agent data found</td></tr>}</tbody></table></FloatingHeaderTable>
        </TableCard.Root>
    </div></DefaultLayout>;
}
