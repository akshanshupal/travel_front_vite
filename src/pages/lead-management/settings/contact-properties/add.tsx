...
import { addContactProperty } from "@/utils/services/contactPropertiesService";
...
export default function ContactPropertiesAddPage() {
    ...
    const validate = () => {
        ...
        if (!form.dataType) next.dataType = "Data type is required";
        setErrors(next);
    ...
    const handleSave = async () => {
        setDirty({ title: true, dataType: true });
        ...
        try {
            ...
            showSnackbar({ title: "Error", description: e?.message || "Failed to add property", color: "danger" });
        } finally {
            setSaving(false);
        }
    };
    ...
    return (
        <DefaultLayout>
            ...
            <TableCard.Root>
                <TableCard.Header
                    title="Add Custom Contact Property"
                    contentTrailing={
                ...
                <div className="bg-primary px-4 py-5 md:px-6">
                    ...
                    {/* Preview box */}
                    <div className="mt-6 rounded-lg border border-secondary bg-secondary/30 p-4">
                        <p className="mb-2 text-xs font-medium uppercase tracking-wide text-tertiary">Preview</p>
                        <div className="flex flex-col gap-1.5">
                            <Label>{form.title || "Property Name"}</Label>
                            <div className="h-9 w-full max-w-xs rounded-md border border-secondary bg-primary px-3 py-2 text-sm text-tertiary">
                                {form.dataType === "boolean" ? (
                                    <span>Yes / No</span>
                                ) : form.dataType === "date" ? (
                                    <span>DD / MM / YYYY</span>
                                ) : (
                                    <span>Enter {dataTypeOptions.find(d => d.id === form.dataType)?.label?.toLowerCase() || "value"}…</span>
                                )}
                            </div>
                        </div>
                    </div>
