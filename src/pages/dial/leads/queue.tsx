...
import { getDialQueue, reserveNextDialLead, type DialQueueItem } from "@/utils/services/dialService";
...
export default function DialLeadsQueuePage() {
    ...
    useEffect(() => {
        setLoading(true);
        getDialQueue({ page, limit, search: query, stage: stage === "all" ? undefined : stage, campaign: campaign || undefined })
            .then((response) => setItems(response?.data || []))
            .catch(() => setItems([]))
            .finally(() => setLoading(false));
    }, [query, page, limit, stage, campaign]);

    useEffect(() => setTotalCount(null), [query, limit, stage]);

    const requestTotalCount = async () => {
        setCountLoading(true);
        try {
            const response = await getDialQueue({ search: query, page: 1, limit: 1, totalCount: true, stage: stage === "all" ? undefined : stage, campaign: campaign || undefined });
    ...
    const stageTitle = stageLabels[stage] || "All leads";
    const [starting, setStarting] = useState(false);
    const startCalling = async () => {
        if (starting) return;
        setStarting(true);
        try {
            const response = await reserveNextDialLead({ stage: stage === "all" ? undefined : stage, campaign: campaign || undefined });
            const leadId = response?.lead?.id;
            if (!leadId) throw new Error("No eligible lead was returned");
            navigate(`/dial/leads/view/${leadId}?token=${encodeURIComponent(response.token || "")}`);
        } catch (error) {
            setItems([]);
        } finally {
            setStarting(false);
    ...
    return (
        <DialShell
            title={stageTitle}
            description="Review the calling queue and open a lead to record the next disposition."
            action={<div className="flex items-center gap-2"><Button color="primary" size="sm" isLoading={starting} onClick={startCalling}>Start Calling</Button><Button color="secondary" size="sm" onClick={() => navigate("/dial/leads")}><span className="flex items-center gap-2"><FiArrowLeft aria-hidden="true" /> My Leads</span></Button></div>}
        >
            <div className="flex gap-3">
                <Input aria-label="Search leads" placeholder="Search By Contact Number" value={query} onChange={(value) => { setQuery(value); setPage(1); }} />
            </div>
