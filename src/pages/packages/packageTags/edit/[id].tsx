...
export default function PackageTagsEditPage() {
    ...
    useEffect(() => {
        const run = async () => {
            ...
            setLoading(true);
            try {
                const res = await fetchWithToken(`/api/packagetag/${id}`, {});
                ...
                showSnackbar({ title: "Error", description: e?.message || "Failed to load package tag", color: "danger" });
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleSave = async () => {
        ...
        setSaving(true);
        try {
            const res = await fetchWithToken(`/api/packagetag/${id}`, { title: form.title.trim(), status: form.status === "true" }, { method: "PUT" });
            if ((res as any)?.error) throw new Error((res as any).error);
            showSnackbar({ title: "Success", description: "Package tag updated", color: "success" });
            navigate("/packages/packageTags");
        } catch (e: any) {
            showSnackbar({ title: "Error", description: e?.message || "Failed to update package tag", color: "danger" });
        } finally {
            setSaving(false);
        }
    };
