...
const statusToLabel = (status: PackageTagItem["status"]) => {
    if (status === true || status === "true" || status === 1 || status === "1") return "Active";
    if (status === false || status === "false" || status === 0 || status === "0") return "Inactive";
    return "—";
};

export default function PackageTagsListPage() {
    const navigate = useNavigate();
    const { pathname, search } = useLocation();
    const initial = parseSearch(search);
    const availableWidth = useAvailableTableWidth();
    const { can } = useAccess();
    const canView = can("packagetag", "view");
    const canAdd = can("packagetag", "add");
    const canEdit = can("packagetag", "edit");
    const canDelete = can("packagetag", "delete");
    ...
    const handleDelete = async () => {
        ...
        try {
            const res = await fetchWithToken(`/api/packagetag/${deleteTarget.id}`, {}, { method: "DELETE" });
            if ((res as any)?.error) {
                throw new Error((res as any)?.error || "Delete failed");
            }
            useStoreSnackbar.getState().showSnackbar({
                title: "Deleted",
                description: "Package tag deleted",
                color: "success",
            });
            ...
            useStoreSnackbar.getState().showSnackbar({
                title: "Error",
                description: e?.message || "Failed to delete package tag",
                color: "danger",
            });
        } finally {
            ...
            <div style={{ width: availableWidth }}>
                <TableCard.Root className="w-full">
                    <TableCard.Header
                        ...
                        contentTrailing={
                            <div className="flex w-full flex-col gap-2 md:w-auto md:flex-row md:items-center">
                                ...
                                <Button size="sm" color="primary" iconLeading={Plus} isDisabled={!canAdd} onClick={() => navigate("/packages/packageTags/add")}>
                                    Add Tag
                                </Button>
                            </div>
                        }
                    ...
                    {loading ? (
                        ...
                        <StickyTable ariaLabel="Package tag list" columns={columns} items={items} availableWidth={availableWidth} loading={loading}>
                            {(item) => (
                                <Table.Row id={item.id} columns={columns}>
                                    {(column) => (
                                        <Table.Cell className={`${column?.className || ""} ${column.id === "actions" ? "whitespace-nowrap" : ""}`.trim()}>
                                            {column.id === "index" ? (
                                                ...
                                                canView ? (
                                                    <button
                                                        type="button"
                                                        onClick={() => navigate(`/packages/packageTags/view/${item.id}`)}
                                                ...
                                                <div className="flex w-full items-center justify-end gap-1.5">
                                                    <ButtonUtility
                                                        ...
                                                        icon={Edit01}
                                                        onClick={() => navigate(`/packages/packageTags/edit/${item.id}`)}
                                                        color="warning"
                                                        isDisabled={!canEdit}
                                                    />
                                                    <ButtonUtility
                        ...
                        <Dialog>
                            <div className="relative w-full rounded-xl bg-primary p-5 ring-1 ring-secondary">
                                ...
                                <div className="space-y-1">
                                    <h2 className="text-lg font-semibold text-primary">Delete package tag</h2>
                                    <p className="text-sm text-tertiary">{deleteTarget?.title ? `Delete "${deleteTarget.title}"?` : "Delete this package tag?"}</p>
                                </div>
