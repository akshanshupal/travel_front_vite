...
export default function PackageTypeAddPage() {
    ...
    const handleSave = async () => {
        ...
        setSaving(true);
        try {
            const res = await fetchWithToken("/api/packagetype", { title: form.title.trim(), status: true }, { method: "POST" });
            if ((res as any)?.error) throw new Error((res as any).error);
            showSnackbar({ title: "Success", description: "Package type created", color: "success" });
            navigate("/packages/packageType");
        } catch (e: any) {
            showSnackbar({ title: "Error", description: e?.message || "Failed to create package type", color: "danger" });
        } finally {
            setSaving(false);
        }
    };
