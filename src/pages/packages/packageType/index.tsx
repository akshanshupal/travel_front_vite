...
const statusToLabel = (status: PackageTypeItem["status"]) => {
...
export default function PackageTypeListPage() {
    ...
    const canEdit = can("packagetype", "edit");
    const canDelete = can("packagetype", "delete");
    ...
    const [items, setItems] = useState<PackageTypeItem[]>([]);
    ...
    useEffect(() => {
        const run = async () => {
            ...
            setLoadError(null);
            try {
                const res = await fetchWithToken("/api/packagetype", {
                    page: String(page),
                    limit: String(limit),
                ...
                const normalized = asArray(list)
                    .map((it: any) => {
                        const id = getId(it);
                        if (!id) return null;
                        return { ...it, id } as PackageTypeItem;
                    })
                    .filter(Boolean) as PackageTypeItem[];
                setItems(normalized);
            } catch (e: any) {
                setLoadError(e?.error?.message || e?.message || "Failed to load package types");
                setItems([]);
            } finally {
                setLoading(false);
            }
    ...
    const viewTotalCount = async () => {
        ...
        try {
            const res: any = await fetchWithToken("/api/packagetype", {
    ...
    const handleDelete = async () => {
        ...
        try {
            const res = await fetchWithToken(`/api/packagetype/${deleteTarget.id}`, {}, { method: "DELETE" });
            if ((res as any)?.error) {
            ...
            useStoreSnackbar.getState().showSnackbar({
                title: "Error",
                description: e?.message || "Failed to delete package type",
                color: "danger",
            });
        } finally {
                    ...
                    <TableCard.Header
                        ...
                        contentTrailing={
                            <div className="flex w-full flex-col gap-2 md:w-auto md:flex-row md:items-center">
                                ...
                                <Button size="sm" color="primary" iconLeading={Plus} isDisabled={!canAdd} onClick={() => navigate("/packages/packageType/add")}>
                                    Add Type
                                </Button>
                            </div>
                        }
                                        ...
                                        <Table.Cell className={`${column?.className || ""} ${column.id === "actions" ? "whitespace-nowrap" : ""}`.trim()}>
                                            {column.id === "index" ? (
                                                ...
                                                canView ? (
                                                    <button
                                                        type="button"
                                                        onClick={() => navigate(`/packages/packageType/view/${item.id}`)}
                                                ...
                                                <div className="flex w-full items-center justify-end gap-1.5">
                                                    <ButtonUtility
                                                        ...
                                                        icon={Edit01}
                                                        onClick={() => navigate(`/packages/packageType/edit/${item.id}`)}
                                                        color="warning"
                                                        isDisabled={!canEdit}
                                                    />
                                                    <ButtonUtility
                        ...
                        <Dialog>
                            <div className="relative w-full rounded-xl bg-primary p-5 ring-1 ring-secondary">
                                ...
                                <div className="space-y-1">
                                    <h2 className="text-lg font-semibold text-primary">Delete package type</h2>
                                    <p className="text-sm text-tertiary">
                                        {deleteTarget?.title ? `Delete "${deleteTarget.title}"?` : "Delete this package type?"}
                                    </p>
                                </div>
