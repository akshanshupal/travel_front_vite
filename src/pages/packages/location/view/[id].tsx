...
export default function PackageLocationViewPage() {
    ...
    useEffect(() => {
        const run = async () => {
            ...
            setLoading(true);
            try {
                const response = await fetchWithToken(`/api/location/${id}`, {});
                if ((response as any)?.error) throw new Error((response as any).error);
                setData((response as any)?.data ?? response);
            } catch (error: any) {
                showSnackbar({ title: "Error", description: error?.message || "Failed to load location", color: "danger" });
            } finally {
                setLoading(false);
            }
        };
                ...
                <TableCard.Header
                    ...
                    contentTrailing={
                        <div className="flex w-full flex-col gap-2 md:w-auto md:flex-row md:items-center md:justify-end">
                            ...
                            <Button color="primary" iconLeading={Edit01} onClick={() => navigate(`/packages/location/edit/${id}`)}>
                                Edit
                            </Button>
                        </div>
                    }
