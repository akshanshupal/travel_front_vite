...
export default function PackageLocationEditPage() {
    ...
    useEffect(() => {
        const run = async () => {
            if (!id) {
                setLoading(false);
                showSnackbar({ title: "Error", description: "Invalid location id", color: "danger" });
                return;
            }
            setLoading(true);
            try {
                const res = await fetchWithToken(`/api/location/${id}`, {});
                ...
                showSnackbar({ title: "Error", description: e?.message || "Failed to load location", color: "danger" });
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleSave = async () => {
        ...
        setSaving(true);
        try {
            const res = await fetchWithToken(`/api/location/${id}`, { title: form.title.trim(), status: form.status === "true" }, { method: "PUT" });
            if ((res as any)?.error) throw new Error((res as any).error);
            showSnackbar({ title: "Success", description: "Location updated", color: "success" });
            navigate("/packages/location");
        } catch (e: any) {
            showSnackbar({ title: "Error", description: e?.message || "Failed to update location", color: "danger" });
        } finally {
            setSaving(false);
        }
    };
    ...
    const breadcrumbs = (
        <div className="mb-4 rounded-sm border border-secondary bg-primary p-2">
            <div className="flex flex-wrap items-center gap-1 text-sm text-tertiary">
                ...
                <span>/</span>
                <button type="button" onClick={() => navigate("/packages/location")} className="rounded-sm px-1 py-0.5 text-primary hover:bg-primary_hover">
                    Location
                </button>
                <span>/</span>
                ...
                <TableCard.Header
                    ...
                    contentTrailing={
                        <div className="flex w-full flex-col gap-2 md:w-auto md:flex-row md:items-center md:justify-end">
                            <Button color="secondary" iconLeading={ArrowLeft} onClick={() => navigate("/packages/location")}>
                                Back
                            </Button>
