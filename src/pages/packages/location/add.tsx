...
export default function PackageLocationAddPage() {
    ...
    const handleSave = async () => {
        ...
        setSaving(true);
        try {
            const res = await fetchWithToken("/api/location", { title: form.title.trim(), status: true }, { method: "POST" });
            if ((res as any)?.error) throw new Error((res as any).error);
            showSnackbar({ title: "Success", description: "Location created", color: "success" });
            navigate("/packages/location");
        } catch (e: any) {
            showSnackbar({ title: "Error", description: e?.message || "Failed to create location", color: "danger" });
        } finally {
            setSaving(false);
        }
    };
