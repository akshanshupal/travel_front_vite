...
export const MultiSelectBase = ({
    ...
    useEffect(() => {
        ...
        accessibleList.items.forEach((item) => accessibleList.remove(item.id));
        nextItems.forEach((item) => accessibleList.append(item));
    }, [items]);
        ...
        <div className="relative flex w-full flex-1 flex-row flex-wrap items-center justify-start gap-1.5">
            ...
            <div className={cx("relative flex min-w-[20%] flex-1 flex-row items-center", !isSelectionEmpty && "ml-0.5", shortcut && "min-w-[30%]")}>
                <AriaInput
                    ...
                    className="w-full flex-[1_0_0] appearance-none bg-transparent text-md text-ellipsis text-primary caret-alpha-black/90 outline-none placeholder:text-placeholder focus:outline-none focus:ring-0 border-none disabled:cursor-not-allowed disabled:text-disabled disabled:placeholder:text-disabled"
                />
    ...
    return (
        <AriaGroup
            ...
            className={({ isFocusWithin, isDisabled }) =>
                cx(
                    "relative flex w-full items-center gap-2 rounded-lg bg-primary shadow-xs outline-hidden transition duration-100 ease-linear ring-inset",
                    isDisabled && "cursor-not-allowed bg-disabled_subtle",
                    isFocusWithin ? "ring-2 ring-brand" : "ring-1 ring-primary",
                    sizes[size].root,
                )
            }
        >
