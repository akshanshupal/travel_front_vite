    ...
    return (
        <AriaGroup
            ...
            className={({ isFocusWithin, isDisabled, isInvalid }) =>
                cx(
                    "relative flex w-full flex-row place-content-center place-items-center rounded-lg bg-[var(--color-bg-primary)] shadow-xs ring-1 ring-primary transition-shadow duration-100 ease-linear ring-inset",
        ...
        >
            ...
            <AriaInput
                ...
                className={cx(
                    "m-0 w-full bg-transparent text-md text-primary ring-0 outline-hidden placeholder:text-placeholder autofill:rounded-lg autofill:text-primary dark:[color-scheme:dark]",
            ...
            {shortcut && (
                <div
                    className={cx(
                        "pointer-events-none absolute inset-y-0.5 right-0.5 z-10 flex items-center rounded-r-[inherit] bg-linear-to-r from-transparent to-[var(--color-bg-primary)] to-40% pl-8",
...
export const Input = ({
    ...
    tooltipClassName,
    ...props
}: InputProps & Omit<React.InputHTMLAttributes<HTMLInputElement>, "size" | "value" | "onChange" | "defaultValue">) => {
    const { type, onKeyDown, min, max, step, pattern, maxLength, minLength, ...textFieldProps } = props;
    
    return (
        <TextField aria-label={!label ? placeholder : undefined} {...textFieldProps} className={className}>
