...
const styles = sortCx({
    ...
    rootSelected: "bg-active hover:bg-secondary_hover text-brand-secondary",
});
...
interface NavItemBaseProps {
    ...
    /** Content to display. */
    /** Additional CSS classes to apply to the nav item. */
    className?: string;
    /** Content to display. */
    children?: ReactNode;
}

export const NavItemBase = ({ current, type, open, badge, href, icon: Icon, children, truncate = true, onClick, className }: NavItemBaseProps) => {
    ...
    const labelElement = (
        <span
            className={cx(
                ...
                truncate && "truncate",
                current && "text-brand-secondary font-semibold",
            )}
        >
            {children}
        </span>
        ...
        return (
            <summary className={cx("px-3 py-2", styles.root, current && styles.rootSelected, className)} onClick={onClick}>
                ...
                <ChevronDown
                    aria-hidden="true"
                    className={cx(
                        "ml-3 size-4 shrink-0 stroke-[2.5px] text-fg-quaternary transition-transform duration-200",
                        open && "-scale-y-100",
                    )}
                />
            </summary>
