...
interface SidebarNavigationSlimProps {
    ...
    /** Whether to show the mobile navigation header. */
...
export const SidebarNavigationSlim = ({
    ...
    const matchesItem = (item: NavItemType, url?: string): boolean => {
        if (!url) return false;
        if (item.href === url) return true;
        return Boolean(item.items?.some((child) => matchesItem(child, url)));
    };

    const activeItem = [...items, ...footerItems].find((item) => matchesItem(item, activeUrl));
    ...
    const handleMainSidebarOver = (e: PointerEvent<HTMLElement>) => {
        const target = e.target as HTMLElement;
        if (!target.closest("[data-menu-list]")) {
            setCurrentItem(activeItem || items[1]);
        }
    };

    const mainSidebar = (
        <aside
            onPointerOver={handleMainSidebarOver}
            style={{
        ...
        >
            ...
            >
                ...
                <ul className="mt-4 flex flex-col gap-0.5 px-3" data-menu-list>
                    {items.map((item) => (
                ...
                <div className="mt-auto flex flex-col gap-4 py-5">
                    {footerItems.length > 0 && (
                        <ul className="flex flex-col gap-0.5 px-3" data-menu-list>
                    ...
                    <div className="px-3">
                        ...
                        >
                            <Avatar status="online" src={userAvatarSrc} initials={userInitials} size="md" alt={storeUser?.name || "User"} />
                        </AriaButton>
                        <AriaPopover
    ...
    const renderSecondaryItem = (item: NavItemType, depth: number): React.ReactNode => {
            ...
            return (
                <li key={`${item.label}-${depth}`} className="py-0.5">
                    <div className={paddingClass}>
                        <NavItemBase href={item.href} icon={item.icon} badge={item.badge} type="collapsible">
                            {item.label}
                        </NavItemBase>
                    </div>
                    <div>
                        <ul className={cx("py-0.5", paddingClass)}>{item.items?.map((child) => renderSecondaryItem(child, depth + 1))}</ul>
                    </div>
                </li>
            );
        }
            ...
            {isSecondarySidebarVisible && (
                ...
                >
                    <div style={{ width: SECONDARY_SIDEBAR_WIDTH }} className="flex h-full flex-col px-4 pt-6">
                        ...
                        <ul className="py-2">{currentItem.items?.map((item) => renderSecondaryItem(item, 0))}</ul>
                        <div className="sticky bottom-0 mt-auto flex justify-between border-t border-secondary bg-primary px-2 py-5">
                            <div>
                                <p className="text-sm font-semibold text-primary">{storeUser?.name || "—"}</p>
                                <p className="text-sm text-tertiary">{storeUser?.email || "—"}</p>
                            </div>
